...
async function checkEmailDefault() {
    const { error } = await supabase.from('email_templates_default').select('*').limit(1);
    if (error) {
        console.log('email_templates_default does NOT exist (or error):', error.message);
    } else {
        console.log('email_templates_default EXISTS.');
    }
}
