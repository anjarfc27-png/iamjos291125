...
export async function moveMenuItem(itemId: string, direction: 'up' | 'down') {
    ...
    try {
        ...
        // 2. Find adjacent item to swap with
        let adjacentQuery = supabase
            .from('navigation_menu_items')
            .select('*')
            .eq('navigation_menu_id', currentItem.navigation_menu_id)
            .order('seq', { ascending: direction === 'down' });

        if (direction === 'up') {
            adjacentQuery = adjacentQuery.lt('seq', currentItem.seq).limit(1);
        } else {
            adjacentQuery = adjacentQuery.gt('seq', currentItem.seq).limit(1);
        }

        const { data: adjacentItems } = await adjacentQuery;
        const adjacentItem = adjacentItems?.[0];

        if (!adjacentItem) return { success: false, message: "Cannot move further" };

        // 3. Swap seq
        const { error: updateError1 } = await supabase
            .from('navigation_menu_items')
            .update({ seq: adjacentItem.seq })
