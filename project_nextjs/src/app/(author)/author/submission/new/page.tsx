  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      try {
        ...
        // 2. Insert into submission_files
        // We need a genre_id (e.g., Article Text). For now we can leave it null or fetch a default.
        ...
        setFormData(prev => ({ ...prev, files: [...prev.files, file] })); // We might want to store more info than just File object
      } catch (error: any) {
      ...
      try {
        ...
        const { data: publication, error: pubError } = await supabase
          ...
          .insert({
            ...
            status: 1, // Queued
            version: 1,
            primary_locale: formData.language,
          })
          .select()
          .single();
        ...
        // 3. Update Submission with current_publication_id
        await supabase
          .from('submissions')
