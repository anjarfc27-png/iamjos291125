...
import { AssistantSideNav } from "@/components/assistant/side-nav";
...
export default function AssistantLayout({ children }: Props) {
  ...
  useEffect(() => {
    ...
    if (!user) {
      setAuthorized(false);
      router.replace("/login?source=/assistant");
      return;
    }
    const canAccess = user.roles?.some(r => r.role_path === "assistant" || r.role_path === "admin");
    if (!canAccess) {
      setAuthorized(false);
